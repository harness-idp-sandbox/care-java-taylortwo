...
# run app
java -jar target/java-taylortwo-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/java-taylortwo:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/java-taylortwo -n care

# get external endpoint
kubectl get svc java-taylortwo-svc -n care
```
...
```bash
kubectl port-forward svc/java-taylortwo-svc 8080:80 -n care
...
kubectl rollout restart deploy/java-taylortwo -n care
kubectl rollout status deploy/java-taylortwo -n care
```
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/javataylortwo/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javataylortwo/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© Taylor.Hays@carelumen.com. For demo/PoV purposes.
